...
<Step title="Respect `Retry-After` and the `RateLimit-*` headers">
On `rate_limited` responses, the `Retry-After` header tells you how many seconds to wait. The per-route `RateLimit-Limit` / `RateLimit-Remaining` / `RateLimit-Reset` headers (IETF draft naming) show how much budget remains in the current window when present. The always-present `X-RateLimit-Limit` is the static ceiling for the route. Honor whichever is present — retrying earlier just burns budget.
</Step>
