...
- **The write is conditional on the row still being `queued`.** The minimum
  schedule delay is one second, so a job can fire between the read and the
  write; an edit that loses that race returns `409` instead of rewriting the
  text of a post that already went out.
