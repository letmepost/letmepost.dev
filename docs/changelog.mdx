---
title: Changelog
description: Reverse-chronological release notes covering API surface, schema, breaking changes, and new platforms.
...
### Added: edit a queued post's caption and media

`PATCH /v1/posts/:id` took `scheduledAt` and nothing else, so the only way to fix
a typo or swap an image on a scheduled post was to cancel it and compose it
again from scratch. The dashboard drawer even labelled the affordance "Edit"
while offering only reschedule and cancel.

- **`PATCH /v1/posts/:id` now accepts `text` and `media`** alongside
  `scheduledAt`. Every field is optional and at least one must be present. An
  omitted field keeps its stored value; `media: []` clears the attachments.
- **Edited content is preflighted** against the target account's platform using
  the *effective* post — the merge of the request and the stored row — so
  replacing a four-image set with a single video is validated as the video-only
  post it becomes. Rejections return `400` with the platform's own rule and
  change nothing. Deep checks (byte size, MIME) still run at publish time, as
  on create.
- **A content-only edit does no queue work.** The publish job carries just the
  post id and the worker re-reads the row at fire time, so only a `scheduledAt`
  change replaces the BullMQ job.
- **The write is a compare-and-swap** over every field preflight validated
  against, not just the status. The minimum schedule delay is one second, so a
  job can fire between the read and the write; and two overlapping edits would
  otherwise each validate the same stale row and merge into a result neither
  one checked. A losing writer gets a `409` and re-reads.
- **Rescheduling removes the old job before it commits, and enqueues the
  replacement after.** A job a worker has already claimed then publishes the
  pre-edit content and the request returns `409` — rather than that job waking
  up and publishing freshly-edited content at the time the caller just moved
  away from. If the replacement cannot be enqueued, the row rolls back, a job
  is restored at the original time, and the request returns `500` instead of a
  success the queue never accepted.
- **New webhook [`post.updated`](/webhooks/post-updated)** with a `changed`
  array naming which content fields moved.
  [`post.rescheduled`](/webhooks/post-rescheduled) is unchanged and stays
  time-only; a request that moves both fires both events.
- **Dashboard**: the scheduled-post drawer now edits the caption and the media
  strip in place, and saves only the fields that actually changed.


### Fixed: scheduled posts stayed queued and never published

Scheduled publishing was broken end to end. Every `scheduledAt` post failed at
the moment it was handed to the queue, on every platform. Immediate publishes
(no `scheduledAt`) were unaffected throughout.

- **Root cause.** Publish jobs were written with `jobId: publish:<postId>`.
  BullMQ rejects a custom job id containing `:` — it uses the colon as its
  internal key separator — unless the id happens to split into exactly three
  parts. `publish:<uuid>` splits into two, so `queue.add()` threw every time.
  Job ids are now dash-joined (`publish-<postId>`).
- **Why it was silent.** The `posts` row is committed before the enqueue, so
  each failure left a row durably `queued` with no job behind it: no delivery
  attempt, no error on the row, no lifecycle event, and a `500` to the caller.
  `POST /v1/posts` now rolls the whole batch back — every accepted target has
  its job removed, its row canceled, and its quota refunded — rather than
  returning an error over a partly-scheduled batch.
- **Recovery.** A new reconciliation sweep runs every 5 minutes and re-drives
  `queued` / `validated` posts that are past due with no live job. Posts more
  than 24h past `scheduledAt` are failed with [`post.failed`](/webhooks/post-failed)
  instead of being sent, so a recovered backlog can't fire days-old posts at
  once. Rows stranded in `publishing` by an interrupted worker are closed out
  the same way.

### Fixed: X (Twitter) connections dying after a token refresh

- **Refresh tokens are no longer dropped.** X is not obliged to return a
  `refresh_token` on every refresh response. Metadata was rebuilt from the
  response alone and persisted wholesale, so the first such response erased the
  stored refresh token; the next refresh failed with "no refresh token stored",
  emitted [`token.revoked`](/webhooks/token-revoked), and the account stayed
  dead until manually reconnected. The previous value now carries forward.
- **Tokens are refreshed inline at publish time** when they're at or past
  expiry, so a gap in the scheduled refresh chain no longer costs a post. This
  is a best-effort backstop and never fails a publish on its own.

<Note>
Accounts disconnected by the old bug can't be restored server-side — the stored
credential is gone. Reconnect the X account once to clear it.
</Note>

### Fixed: valid media rejected on mime type

- **Media type is detected from the bytes**, not from the declared value. The
  base64 path hard-coded `image/jpeg`, the URL path trusted the origin's
  `Content-Type`, and the `mediaId` path trusted the stored row — so a valid
  JPEG served as `application/octet-stream` was rejected by preflight before it
  ever reached the platform. Detection covers JPEG, PNG, GIF, WebP and ISOBMFF
  containers, discriminated by brand so HEIC and AVIF aren't mistaken for MP4.
- **`POST /v1/media` sniffs on upload**, so the stored `contentType` and the S3
  object's `Content-Type` are correct at the source. Rows written earlier with a
  placeholder type are recovered on read.
- Formats we can't identify still fall back to the declared type, so genuinely
  unsupported media is still rejected with the same rule.

### Fixed: failures that reported nothing

- **`rate_limited` now dispatches [`post.failed`](/webhooks/post-failed).** It
  previously marked the row failed and dispatched no event at all, so a post
  could disappear from an integrator's point of view. It is also terminal now
  rather than consuming retries against a 30-day window.
- **`platformResponse` always carries something.** When an upstream answered
  with an empty body we recorded no detail and discarded the HTTP status,
  producing errors whose own remediation pointed at a field that wasn't there.
  Those now carry `{ httpStatus }` at minimum. Responses with a body are
  unchanged.

### Changed: X launch cap counts calls that reached X

The per-account launch cap (`twitter.launch_cap.per_account`) now counts posts
that actually reached X — published tweets, plus failures X itself returned.
Locally-produced failures such as preflight rejections no longer consume a slot.
Previously any failure counted, which made the cap self-reinforcing: a run of
failures could exhaust a 30-day allowance without a single tweet being created.
The window is also measured from publication rather than creation.
...
- **Stable BullMQ jobIds.** `POST /v1/posts` writes scheduled jobs with a deterministic per-post job id so the cancel/reschedule endpoints can find and replace by post id without tracking BullMQ-assigned ids on the row. (The colon-joined form shipped here was invalid and broke scheduled publishing outright — see [2026-08](#2026-08).)
