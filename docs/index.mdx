...
The API surface is identical across every platform — same `POST /v1/posts`, same error envelope, same idempotency contract. Each platform's launch is gated on its own developer-portal review; approvals clear at different rates. Bluesky is live for everyone today (no platform review). The rest ship as approvals clear — track [the changelog](/changelog) and the public version tracker at `GET /v1/platform-versions`.
