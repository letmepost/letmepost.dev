...
const mintBody = z.object({
   ...
   * anonymous — the shared admin key cannot tell us who is behind a request.
   */
      ...
      consumeImpersonationToken: createAuthEndpoint(
        ...
        async (ctx) => {
          ...
          const [grant] = await db
            ...
            .set({
              usedAt: new Date(),
              consumedIp: clientIp(ctx.headers ?? new Headers()),
            })
            .where(
              and(
          ...
          // it. Both are set at creation rather than patched after, so no
          // window exists where the session looks like a normal login with a
          // normal expiry. dontRememberMe=true keeps it off the long-lived
          // "remember me" path.
