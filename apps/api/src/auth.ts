...
Kamal`,
  ...
  session: {
    additionalFields: {
      // Set only by the admin impersonation plugin. `input: false` keeps it out
      // of any client-writable payload — a caller must never be able to claim,
      // or clear, impersonation on its own session.
