 ...
 * operator can forget whose account they are looking at — and no way to mistake
 * a support session for their own.
 *
 * `impersonatedBy` is set server-side by the admin impersonation plugin and is
 * `input: false`, so a client cannot fake or clear it.
 */
export function ImpersonationBanner() {
  ...
  async function exit() {
    setExiting(true);
    // Ending the impersonated session is a plain sign-out: the session row is
    // the impersonation, so revoking it is the whole exit.
  ...
  return (
    ...
    >
      ...
      <span>
        ...
        impersonation by <strong>{impersonatedBy}</strong>. Actions you take are
